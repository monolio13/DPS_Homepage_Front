...
export default function AdminQA() {
  ...
  const confirmBeforeChange = async (nextAction: () => void) => {
    if (isDirty()) {
      const confirmed = window.confirm(
        "현재 작성 중인 내용이 있습니다. 변경하시겠습니까? 내용은 저장되지 않습니다."
      );
      if (!confirmed) return;
    }
    nextAction();
  ...
  const handleEdit = (qa: QA) => {
    confirmBeforeChange(() => {
      ...
      setAnswer(qa.answer);
      setEditId(qa._id);
      setInitialQuestion(qa.question);
      setInitialAnswer(qa.answer);
    });
  };
...
return (
    <AdminLayout>
      ...
      {/* List Section */}
      <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-200">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">등록된 Q&A 목록</h2>
        {qas.length === 0 ? (
          <p className="text-gray-500">등록된 Q&A가 없습니다.</p>
        ) : (
          <ul className="space-y-4">
